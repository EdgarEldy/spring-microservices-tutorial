 ...
 * observed here (e.g. the {@code unit_price > 0} CHECK constraint) matches
 * what runs in dev/prod rather than an in-memory database that would mask
 * PostgreSQL-specific SQL dialect differences. Copied verbatim from
 * {@code auth-service}'s test config of the same name (never shared via
